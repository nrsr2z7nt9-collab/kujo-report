...
function bind(){
 ...
 $('#pdf-btn').onclick=$('#print-btn').onclick=()=>goPrint(state.id);
 $('#docx-btn').onclick=$('#backup-btn').onclick=backup;$('#restore-btn').onclick=()=>$('#restore-file').click();$('#restore-file').onchange=restore;
 $$('.photo-slot').forEach(el=>{const n=+el.dataset.slot,input=$('input[type=file]',el);input.onchange=async()=>{if(input.files[0]){try{state.photos[n]=await normalize(input.files[0]);renderPhoto(n,state.photos[n]);toast(`写真 ${n} を選択しました`);}catch{alert('この写真を読み込めませんでした。別の写真を選んでください。');}}input.value='';};$('.photo-clear',el).onclick=async e=>{e.stopPropagation();delete state.photos[n];if(state.id)await del('photos',`${state.id}:${n}`);renderPhoto(n);};});
}
...
async function normalize(file){let image,url;try{if('createImageBitmap'in window){image=await createImageBitmap(file,{imageOrientation:'from-image'});}else{url=URL.createObjectURL(file);image=new Image;await new Promise((ok,no)=>{image.onload=ok;image.onerror=no;image.src=url;});}const w=image.width||image.naturalWidth,h=image.height||image.naturalHeight,scale=Math.min(1,2560/Math.max(w,h)),c=document.createElement('canvas');c.width=Math.round(w*scale);c.height=Math.round(h*scale);c.getContext('2d').drawImage(image,0,0,c.width,c.height);const blob=await new Promise(ok=>c.toBlob(ok,'image/jpeg',.9));return blob||file;}catch{return file;}finally{if(url)URL.revokeObjectURL(url);}}
function goPrint(id){if(id)location.href=`print.html?id=${id}`;}
async function backup(){const records=await all('records'),photos=await all('photos'),encoded=[];for(const p of photos)encoded.push({...p,blob:await blob64(p.blob)});const blob=new Blob([JSON.stringify({version:1,records,photos:encoded})],{type:'application/json'}),file=new File([blob],'捕獲報告書バックアップ.json',{type:'application/json'}),share={files:[file],title:'捕獲報告書バックアップ'};if(navigator.share&&(!navigator.canShare||navigator.canShare(share))){try{await navigator.share(share);return;}catch(e){if(e.name==='AbortError')return;}}download(blob,file.name);}
